...
db_container_port=$(grep ^container_port CONF_FILE_PATH | awk -F "=" '{print $2}')
...
db_password=$(grep ^db_password $CONF_FILE_PATH | awk -F "=" '{print $2}')
...
sed -i "s,server.port.*,server.port=8085," $APPLICATION_PROPERTIES_FILE_PATH